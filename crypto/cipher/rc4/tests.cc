...
struct RC4TestVector {
    const char *key;
    const char *output;
};

/**
 * Test vectors specified in RFC 6229.
 */
RC4TestVector RC4TestVectors[] = {
    { "0102030405",
...
// IETF test vectors pick bits from different locations of the stream
// This lists the locations at which those are picked
uint32_t RC4TestVectorLayout[] = { 0x00,  0x10,  0xf0,  0x100, 0x1f0, 0x200,
...
void test_ietf_vectors(crypto::RC4Base *rc4) {
    ...
    size_t num_fragments = sizeof(RC4TestVectorLayout) / sizeof(RC4TestVectorLayout[0]);

    // Length of a single fragment (divided by two because strlen checks hex
    // representation)
    size_t fragment_len = strlen(RC4TestVectors[0].output) / num_fragments / 2;

    for (size_t i = 0; i < sizeof(RC4TestVectors) / sizeof(RC4TestVectors[0]); i++) {
        RC4TestVector &vec = RC4TestVectors[i];
        crypto::bytestring key = crypto::bytestring::from_hex(vec.key);
        crypto::bytestring expected = crypto::bytestring::from_hex(vec.output);
        ...
        for (size_t j = 0; j < num_fragments; j++) {
            crypto::bytestring target(expected.ptr() + j * fragment_len, fragment_len);
            crypto::bytestring actual(stream.ptr() + RC4TestVectorLayout[j], fragment_len);
